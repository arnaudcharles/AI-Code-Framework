You are resuming an existing Puppet module project.
...
- Ensure Puppet documentation coverage explicitly includes manifests, classes, defined types, templates, files, metadata, Hiera, tests, CI, and release.
- Update PROJECT.md with: current state, risks/technical debt, classified findings, quick wins, and recommended plan.
- Say READY when the base is stable.
- Propose /compact when useful, then say NEWCHAT when context is around 60 percent or the thread becomes inefficient.
